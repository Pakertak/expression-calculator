...
function expressionCalculator(expr) {
    ...
    for(let i=0; i<expr.length; i++) 
    {
        if(expr[i]=='(') flag++; 
            ...
            if(expr[i]==')'){ flag--; if(flag<0) throw "ExpressionError: Brackets must be paired";}
    ...
    function addPostFix(str)
    {
        ...
        for(let i = 0; i < str.length; i++)
        {
            ch = str[i];
            if(ch >= '0' && ch <= '9') {
                strout[n] += String(ch); continue;
            }
            if(!sp.length||ch=='('){
                sp.push(ch); if(strout[n]!=''){strout.push(''); n++;} continue;
            }
            if(ch==')')
            {
    ...
    function result(str){
        ...
        for(i = 0; i < str.length; i++)
        {
            ...
            if(!isNaN(Number('+' + ch)))
            {
                sp.push(Number(ch)); continue;
            }
            inf2=sp.pop();
            inf1=sp.pop();
            switch(ch)
            {
                case '+': sp.push(inf1+inf2); break;
                case '-': sp.push(inf1-inf2); break;
                case '*': sp.push(inf1*inf2); break;
                case '/': if(inf2==0) throw "TypeError: Division by zero."; sp.push(inf1/inf2); break;
            }
